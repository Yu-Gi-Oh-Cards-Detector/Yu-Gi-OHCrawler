...
for d in dir_list:
    ...
    SubElement(root, "folder").text = STATIC
    SubElement(root, "filename").text = f"{d}.jpg"
    SubElement(root, "path").text = f"{STATIC}/{d}/{d}.jpg"
    ...
    SubElement(bndbox, "xmin").text = "49"
    SubElement(bndbox, "ymin").text = "273"
    SubElement(bndbox, "xmax").text = "382"
    SubElement(bndbox, "ymax").text = "596"

    tree = ElementTree(root)
    formatted = minidom.parseString(ET.tostring(root)).toprettyxml()
    formatted = formatted.replace(XML_VERSION_STRING, "")
    with open(f"{STATIC}/{d}/{d}.xml", "w", encoding="utf-8") as f:
        f.write(formatted)
